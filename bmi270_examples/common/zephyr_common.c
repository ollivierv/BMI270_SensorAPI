...
{
    struct coines_intf_config intf_info = *(struct coines_intf_config *)intf_ptr;
    return coines_write_spi((enum coines_spi_bus)intf_info.bus, intf_info.dev_addr, reg_addr, (uint8_t *)reg_data, (uint16_t)len);
}
*/
...
int8_t bmi2_interface_init(struct bmi2_dev *bmi, uint8_t intf)
{
    ...
    if (bmi != NULL)
    {
        ...
        if (intf == BMI2_I2C_INTF)
        {
          ...
          __ASSERT(false, "only i2c is implemented");
        }
